...
exports.getEmails = async (req, res) => {
  ...
  async function getAllMessages(labelIds = null, maxResults = 5000, query = '') {
    ...
    do {
      ...
      console.log(`📄 Page ${pageCount} - ${messages.length} messages récupérés (total: ${allMessages.length})`);
  ...
  try {
    ...
    if (gmailQuery) {
      ...
      if (afterDate) console.log(`📅 Mode sync incrémentale depuis: ${new Date(parseInt(afterDate)).toISOString()}`);
    } else {
    ...
    const inboxMessages = (await getAllMessages(['INBOX'], 5000, gmailQuery)).map(m => ({ ...m, _type: 'recu' }));
    ...
    const sentMessages = (await getAllMessages(['SENT'], 5000, gmailQuery)).map(m => ({ ...m, _type: 'envoye' }));
    ...
    const allMailMessages = (await getAllMessages(null, 5000, gmailQuery)).map(m => ({ ...m, _type: 'archive' }));
    ...
    console.log(`✅ ${displayEmails.length} emails affichés (sur ${allMessages.length} disponibles)`);
...
exports.getEmailCount = async (req, res) => {
  ...
  try {
    ...
    console.log(`📬 Polling count: ${newCount} messages (afterDate: ${afterDate ? new Date(parseInt(afterDate)).toISOString() : 'aucun'})`);
    res.json({ newCount });
...
exports.sendReply = async (req, res) => {
  ...
  if (!to || !body || !threadId || !messageId) {
    return res.status(400).json({ error: 'Champs requis manquants (to, body, threadId, messageId)' });
  }
